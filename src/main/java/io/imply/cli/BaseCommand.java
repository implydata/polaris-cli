...
public abstract class BaseCommand {
    ...
    public String postJson(String content, String path, Global global) throws IOException {
        ...
        return post(requestEntity, path, global);
    }
    ...
    public String upload(File file, String path, Global global) throws IOException{
        ...
        HttpEntity reqEntity = MultipartEntityBuilder.create()
                ...
                .build();
        return post(reqEntity, path, global);
    }

    private String post(HttpEntity reqEntity, String path, Global global) throws IOException{
        try( CloseableHttpClient httpclient = HttpClientBuilder.create()
                .setDefaultRequestConfig(config).build()){
            String uri = buildEndpoint(global.environment.name(), global.organization);
            HttpPost post = new HttpPost(uri + path);
            post.setHeader("Authorization", "Bearer " + global.token);
